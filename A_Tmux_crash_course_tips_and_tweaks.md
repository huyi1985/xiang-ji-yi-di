日志包含需要大量挖掘的信息，远不止日志抽取那么简单。本文用基本和高级两种方式举例说明对日志分析的方法以及高级分析的环境搭建。起初用几个命令行、后面用日志管理工具日志自动地让繁重的任务以流水线方式处理。（@网路冷眼 推荐） **[阅读原文 »](http://www.loggly.com/ultimate-guide/logging/analyzing-linux-logs/)**
...
1. Tmux basics.
1. The best of Tmux 最棒的功能
	1. 窗口（Window）
	1. 窗格（Pane）
	1. 会话（Session）
	1. 可以快速地在文本间移动光标或复制文本
	1. 提供了结对编程的功能 And a very neat pair programming feature
1. Tweaks to improve Vim integration. 对进行微调以～
	1. Colorscheme background
	1. Static cursor shape
	1. Indentation at pasting
1. A few extras to enhance the Tmux experience.
